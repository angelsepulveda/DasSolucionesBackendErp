...
namespace Membership.Modules.Features.Update;
...
internal sealed class UpdateModuleHandler(
    ...
    IModuleQueryRepository moduleQueryRepository) : ICommandHandler<UpdateModuleCommand>
{
    public async Task<Unit> Handle(UpdateModuleCommand request, CancellationToken cancellationToken)
    {
        ...
        if (moduleUpdated is null || moduleUpdated.Status) throw new ModuleNotFoundException(request.Payload.Id);
